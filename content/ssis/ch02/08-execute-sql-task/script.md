...
The Execute SQL Task is one of the tasks you'll reach for in nearly
...
question mark as the marker, with parameters mapped by their ordinal
position: zero, one, two, and so on — ODBC starts at one instead of
zero. ADO.NET connections are different — they use named markers like
"at-parameter-name" directly in the statement. Get the wrong marker
...
once per row — a pattern you'll use constantly. Next lesson, we look at
the Execute Package Task — how one package calls another as part of
its own workflow.
