# Script — The Slowly Changing Dimension Transformation

## Segment 1 (title)

Last lesson we built an incremental load pattern by hand out of a Lookup
and a Conditional Split. This lesson, we look at a transformation that's
...
a minimal placeholder record so the fact load can proceed, then update
it for real once the actual dimension data arrives. Next lesson, we leave
the data flow entirely and step into Script Tasks and Script Components —
where C# code, not a drag-and-drop transformation, does the work.
